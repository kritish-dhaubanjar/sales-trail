...
import { deleteTransfer, getTransfers } from '@/services/transfer.service';
...
function Transfer() {
  ...
  const { data, refetch, isFetching } = useQuery({
    queryKey: ['transfers', pagination, debouncedQuery],
    enabled: true,
    ...
    queryFn: () => {
      ...
      return getTransfers({ page, limit, query: debouncedQuery });
    },
  });
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'To A/C',
        cell: ({ row }) => <div>{row.original.to_account.name}</div>,
      },
      {
        accessorKey: 'amount',
        header: 'Amount',
  ...
  const { mutate: onDelete } = useMutation(deleteTransfer, {
    onSuccess: () => {
      deleteRowRef.current = deleteRow;

      refetch();
      toast({ title: `Transfer "${deleteRowRef.current?.title}" successfully deleted.` });
  ...
  return (
    <div className="flex">
      ...
      <Alert
        ...
        description={`This action cannot be undone. This will permanently delete ${deleteRow?.title || deleteRowRef.current?.title} from our servers.`}
      />
...
export default dynamic(() => Promise.resolve(Transfer), { ssr: false });

...
function POSTable() {
  ...
  const { mutate: onDelete } = useMutation(deleteTable, {
    onSuccess: () => {
      deleteRowRef.current = deleteRow;

      refetch();
      toast({ title: `Table "${deleteRowRef.current?.name}" successfully deleted.` });
  ...
  return (
    <div className="flex">
      ...
      <div className="min-h-lvh w-full px-10 py-10">
        ...
        <div className="my-4">
          <h1 className="text-2xl font-bold">Tables ({data?.data?.total ?? 0})</h1>
          <p className="text-xs text-gray-600">
            Use the filter input to quickly search and display specific tables by name, status, or
...
export default dynamic(() => Promise.resolve(POSTable), { ssr: false });

...
function Purchase() {
  ...
  const { mutate, isLoading } = useMutation(createPurchase, {
    onSuccess: (data) => {
      toast({ title: 'Purchase successfully created.' });

      window.open(`/purchases/print?id=${data.data.id}`, '_blank');
      ...
      <div className="min-h-lvh w-full px-10 py-10">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>Create Purchases</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>

        <div className="my-4">
          <h1 className="text-2xl font-bold">Create Purchases</h1>
        </div>

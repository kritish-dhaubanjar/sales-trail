...
function Purchase() {
  ...
  const { data, refetch, isFetching } = useQuery({
    queryKey: ['purchases', pagination, debouncedQuery],
    enabled: true,
    ...
    queryFn: () => {
      ...
      return getPurchases({ page, limit, query: debouncedQuery });
    },
  });
  ...
  const columns = useMemo(
    () => [
            ...
            <Popover>
              ...
              <PopoverContent className="min-w-[600px]">
                <Table>
                  ...
                  <TableBody>
                    {row.original?.purchase_items?.map((purchase, index) => (
                      <TableRow key={purchase.id}>
                        ...
                        <TableCell>{purchase.item.unit.name}</TableCell>

                        <TableCell>{formatter.format(purchase.price)}</TableCell>
      ...
      },
      {
        ...
        enableHiding: false,
        cell: ({ row }) => {
          const totalPurchase = Number(row.original.grand_total);
          const totalTransaction = row.original.transactions.reduce(
            (acc, { amount }) => acc + Number(amount),
            0,
          );

          return totalPurchase === totalTransaction ? <></> : <BellIcon className="text-red-500" />;
        },
      },
      {
        id: 'actions',
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem asChild>
                  <Link
                    href={{ pathname: '/purchases/print', query: { id: row.original.id } }}
                    target="_blank"
                  >
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem asChild>
                  <Link href={{ pathname: '/purchases/edit', query: { id: row.original.id } }}>
  ...
  const { mutate: onDelete } = useMutation(deletePurchase, {
    onSuccess: () => {
      refetch();
      toast({ title: `Purchase successfully deleted.` });
  ...
  return (
    <div className="flex">
      ...
      <Alert
        ...
        description={`This action cannot be undone. This will permanently delete purchase from our servers.`}
      />
      ...
      <div className="min-h-lvh w-full px-10 py-10">
        ...
        <div className="my-4">
          <h1 className="text-2xl font-bold">Purchases ({data?.data?.total ?? 0})</h1>
          ...
          <div className="flex items-center justify-between py-4">
            ...
            <div className="flex items-center space-x-6 lg:space-x-8">
              <div className="flex items-center space-x-2">
                ...
                >
                  <SelectTrigger className="h-[30px!important] w-[70px]">
...
export default dynamic(() => Promise.resolve(Purchase), { ssr: false });

...
const schema = z.object({
  ...
  date: z.string({ required_error: 'A date of purchase is required.' }),
  items: z.array(
    z.object({
...
function Purchase() {
  ...
  useEffect(() => {
    ...
    purchase.items = purchase.purchase_items;
  ...
  const { mutate, isLoading } = useMutation(updatePurchase, {
    onSuccess: () => {
      toast({ title: 'Purchase successfully updated.' });
      ...
      <div className="min-h-lvh w-full px-10 py-10">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>Edit Purchases</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>

        <div className="my-4">
          <h1 className="text-2xl font-bold">Edit Purchases</h1>
        </div>

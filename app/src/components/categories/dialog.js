...
export function CategoryDialog({ open = true, row = null, refetch = () => { }, onClose = () => { } }) {
  ...
  useEffect(() => {
    const defaultValue = row || DEFAULT_CATEGORY;
    reset(defaultValue);
  }, [row, reset, open]);

  const { mutate, isLoading } = useMutation(
    (data) => {
      const mutation = Boolean(data.id) ? updateCategory : createCategory;
    ...
    },
    {
      onSuccess: (response) => {
        refetch();
        onClose();
        toast({ title: `Category "${response.data.name}" successfully saved.` });
      },
      onError: (error) => {
        toast({
          variant: 'destructive',
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{Boolean(row) ? 'Edit' : 'Add'} Category</DialogTitle>
